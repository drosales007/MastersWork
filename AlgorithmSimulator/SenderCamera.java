...
public class SenderCamera extends Process implements Camera {
    ...
    public synchronized void handleMsg(Msg m, int src, String tag){
        if (tag.equals("marker")) {
            ...
            //System.out.println("Application Message : M= "+m +" message ="+ m.getMessage());
            StringTokenizer st = new StringTokenizer( m.getMessage()+"#");
            
            int seqNo = Integer.parseInt(st.nextToken());
            System.out.println("Application Message : M= "+m +" message ="+ m.getMessage() +" St = " + seqNo);
    ...
    public SenderCamera(CameraLinker initComm, Object _app) {
        super(initComm);
        CamUser app = (CamUser) _app;
        this.app = app;
        closed = new boolean[N]; outChan = new MsgList[N];
        for (int i = 0; i < N; i++)
            if (isNeighbor(i)) {
